...
import { getAuthUser, parseJsonBody, toCleanString, generateId } from "@/lib/api-utils";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const cost = order.tokenCost ?? 0;
    if (balance < cost) {
      return { error: "User has insufficient tokens to fulfill this order" as const, status: 409 as const };
    }

    const reason = `Deducted ${cost} tokens for purchasing ${order.itemName} from the shop. ${order.itemName} cost ${cost} tokens. Fulfilled with ${fulfillmentLink}.`;
